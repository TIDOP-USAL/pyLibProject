...
from pyLibGDAL import PostGISTools
...
class Project:
    ...
    def get_project_definition_projected_crs_epsg_code(self):
        str_error = ''
        epsg_code = None
        if not defs_project_definition.PROJECT_DEFINITIONS_TAG_PROJECTED_CRS in self.project_definition:
    ...
    def get_project_definition_projected_crs(self):
        ...
        if not defs_project_definition.PROJECT_DEFINITIONS_TAG_PROJECTED_CRS in self.project_definition:
            str_error =('Not exists {} in project definition'.format(
                defs_project_definition.PROJECT_DEFINITIONS_TAG_PROJECTED_CRS))
            return str_error
        project_crs = self.project_definition[defs_project_definition.PROJECT_DEFINITIONS_TAG_PROJECTED_CRS]
        return str_error, project_crs

    def get_project_definition_tag(self):
        str_error = ''
        tag = None
        if not defs_project_definition.PROJECT_DEFINITIONS_TAG_TAG in self.project_definition:
            str_error =('Not exists {} in project definition'.format(
                defs_project_definition.PROJECT_DEFINITIONS_TAG_TAG))
            return str_error
